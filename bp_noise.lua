...
-- E2 - select voice 1-16
-- E3 - set amplitude
-- K2 + E2 - change filter note
-- K2 + E3 - detune
...
engine.name = "BP_noise"
MusicUtil = require "musicutil"

function init()
	print("loaded BP_noise engine")
	add_params()
	edit = 0
	for i = 1,16 do
...
function add_params()
	...
	for i = 1,16 do
		...
		params:add_control("filter_q" .. i, "filter Q " .. i, controlspec.new(0.01, 1.0, 'lin', 0.01, 0.1))
		params:set_action("filter_q" .. i, function(x) set_q(i - 1, x) end)
...
function set_q(synth_num, value)
	engine.filt_q(synth_num, value)
	edit = synth_num
	redraw()
end
...
function enc(n, delta)
	...
	elseif n == 3 then
		...
		elseif key_1_pressed == 0 and key_2_pressed == 0 and key_3_pressed == 1 then
			-- set the filter q value
			params:set("filter_q" .. edit+1, params:get("filter_q" .. edit+1) + delta * 0.01)
...
function redraw()
	...
	screen.level(15)
	screen.text(params:get("filter_q" .. edit+1))
	screen.move(0,19)
	screen.level(2)
